<header class="navbar bg-base-200 text-base-content p-4">
  <div class="navbar-start">
    <div class="flex items-center">
      <img
        src="{{ asset('i_lab.png') }}"
        alt="i_lab logo"
        width="120"
        height="120"
        class="fill-current my-auto">
    </div>
  </div>
  <div class="navbar-end">
  @auth
    <form method="POST" action="{{ route('logout') }}" class="hidden lg:block">
      @csrf
      <button type="submit" class="btn btn-ghost mr-2 flex items-center justify-center">
        Logout
      </button>
    </form>
  @else
    <a href="{{ route('login') }}" class="btn btn-ghost mr-2 hidden lg:inline-flex items-center justify-center">
      Login
    </a>
  @endauth
  @auth
    @if(auth()->user()->role === 'teacher' || auth()->user()->role === 'admin')
    <div class="hidden lg:flex">
      <a href="{{ route('quizzes.index') }}" class="btn btn-ghost mr-2 hidden lg:inline-flex items-center justify-center">
        Dashboard
      </a>
      
      </a>
    </div>
    @endif
  @endauth
  <label class="swap swap-rotate">
  <input type="checkbox" onchange="toggleTheme(this)" />

  <!-- moon icon -->
  <svg
    class="swap-on h-7 w-7 fill-current"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 24 24">
    <path
      d="M21.64,13a1,1,0,0,0-1.05-.14,8.05,8.05,0,0,1-3.37.73A8.15,8.15,0,0,1,9.08,5.49a8.59,8.59,0,0,1,.25-2A1,1,0,0,0,8,2.36,10.14,10.14,0,1,0,22,14.05,1,1,0,0,0,21.64,13Zm-9.5,6.69A8.14,8.14,0,0,1,7.08,5.22v.27A10.15,10.15,0,0,0,17.22,15.63a9.79,9.79,0,0,0,2.1-.22A8.11,8.11,0,0,1,12.14,19.73Z" />
  </svg>

  <!-- sun icon -->
  <svg
    class="swap-off h-7 w-7 fill-current"
    xmlns="http://www.w3.org/2000/svg"
    viewBox="0 0 24 24">
    <path
      d="M5.64,17l-.71.71a1,1,0,0,0,0,1.41,1,1,0,0,0,1.41,0l.71-.71A1,1,0,0,0,5.64,17ZM5,12a1,1,0,0,0-1-1H3a1,1,0,0,0,0,2H4A1,1,0,0,0,5,12Zm7-7a1,1,0,0,0,1-1V3a1,1,0,0,0-2,0V4A1,1,0,0,0,12,5ZM5.64,7.05a1,1,0,0,0,.7.29,1,1,0,0,0,.71-.29,1,1,0,0,0,0-1.41l-.71-.71A1,1,0,0,0,4.93,6.34Zm12,.29a1,1,0,0,0,.7-.29l.71-.71a1,1,0,1,0-1.41-1.41L17,5.64a1,1,0,0,0,0,1.41A1,1,0,0,0,17.66,7.34ZM21,11H20a1,1,0,0,0,0,2h1a1,1,0,0,0,0-2Zm-9,8a1,1,0,0,0-1,1v1a1,1,0,0,0,2,0V20A1,1,0,0,0,12,19ZM18.36,17A1,1,0,0,0,17,18.36l.71.71a1,1,0,0,0,1.41,0,1,1,0,0,0,0-1.41ZM12,6.5A5.5,5.5,0,1,0,17.5,12,5.51,5.51,0,0,0,12,6.5Zm0,9A3.5,3.5,0,1,1,15.5,12,3.5,3.5,0,0,1,12,15.5Z" />
  </svg>
</label>
<style>
  * {
    transition: all 0s ease;
  }
</style>
<script>
  function toggleTheme(checkbox) {
    const html = document.documentElement;
    const theme = checkbox.checked ? 'corporate' : 'dracula';
    html.setAttribute('data-theme', theme);
    localStorage.setItem('theme', theme);
  }

  // Set initial state based on localStorage
  const savedTheme = localStorage.getItem('theme') || 'corporate';
  document.documentElement.setAttribute('data-theme', savedTheme);
  document.querySelector('.swap input').checked = savedTheme === 'corporate';
</script>
    <div class="dropdown dropdown-end lg:hidden">
      <label tabindex="0" class="btn btn-ghost">
        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h8m-8 6h16" />
        </svg>
      </label>
      <ul tabindex="0" class="menu menu-sm dropdown-content mt-3 z-[1] p-2 shadow bg-base-100 rounded-box w-52">
        @auth
          @if(auth()->user()->role === 'admin' || auth()->user()->role === 'teacher')
            <li><a href="{{ route('quizzes.index') }}" class="link link-hover">Dashboard</a></li>
          @endif
          <li>
            <form method="POST" action="{{ route('logout') }}">
              @csrf
              <button type="submit" class="link link-hover w-full text-left">Uitloggen</button>
            </form>
          </li>
        @else
          <li><a href="{{ route('login') }}" class="link link-hover">Inloggen</a></li>
        @endauth
      </ul>
    </div>
  </div>
</header>